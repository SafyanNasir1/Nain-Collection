...
function SocialIcon({
  ...
  return (
    <a
      ...
      rel="noopener noreferrer"
      aria-label={label}
      className="group relative flex h-7 w-7 items-center justify-center rounded-full border border-white/15 bg-white/10 transition-all duration-300 ease-out hover:-translate-y-1 hover:border-[black] hover:bg-[black] hover:shadow-[0_6px_16px_rgba(201,162,39,0.4)] sm:h-9 sm:w-9"
    >
      <Image
        src={image}
        alt={label}
        width={15}
        height={15}
        className="h-[18px] w-[18px] object-contain opacity-80 transition-all duration-300 ease-out group-hover:scale-110 group-hover:opacity-100"
      />
    </a>
  );
}
